<template>
    <li class="card">
      <h3 class="component-title">Match #{{ match.id }}</h3>
      <p class="details" v-if="match.players.length >= 1">
      ...
      <p class="details" v-if="inProgress || isCompleted">
        {{ match.players[0].score }} : {{ match.players[1].score }}
      </p>
      <p class="details" id="match-time-since-end">{{ timeSinceEnd }}</p>
      <!-- TODO: watch to live update the duration for matches in progress -->
      <p class="details" v-if="inProgress || isCompleted">Duration: {{ duration }}</p>
