...
/*   Updated: 2022/10/23 18:58:19 by pcordeir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::ostream&	Form::write(std::ostream& dest) const {
...
void	Form::beSigned(const Bureaucrat& b_obj) {
	if (is_signed)
		...
		if (grade_to_sign < b_obj.getGrade())
			throw myException("Grade too low!");
		is_signed = true;
		std::cout << b_obj.getName() << " signed " << name << CYAN << " <" << this->getTarget() << ">" << RESET << std::endl;
	}
}

void	Form::execute(const Bureaucrat& executor) const {
	if (executor.getGrade() > grade_to_exec)
		throw myException("Grade to low to execute!");
	else if (!is_signed)
		throw myException("Form not signed!");
	this->action();
	return;
}
